...
export function ViewerPanel({ media, currentPage, zoom, settings, onPdfMeta, onVideoStateChange }: Props) {
  ...
  useEffect(() => {
    ...
    void render();
  }, [pdfDoc, currentPage, zoom, normalizedMedia.mediaType]);
  ...
  return (
    ...
    >
      ...
      {normalizedMedia.mediaType === "image" && (
        <img className="media-image" src={mediaUrl} alt="viewer media" style={{ transform: `scale(${zoom})` }} />
      )}
      {normalizedMedia.mediaType === "video" && <video ref={videoRef} className="media-video" src={mediaUrl} autoPlay controls={false} />}
